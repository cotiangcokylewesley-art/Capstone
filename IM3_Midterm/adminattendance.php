...
<?php include('config/headeradm.php')?>
    ...
    <div class="mainshit">
        ...
        <div class="topbar">
            <form method="GET" action="">
                ...
                <button type="button" class="appli">Export Logs</button>
            </form>
        </div>
                ...
                <?php foreach ($student_data as $student): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <select class="attendance" data-attendance-id="<?php echo $student['attendance_id']; ?>">
                                <option disabled selected value style="display:none">Edit</option>
                                ...
                                <option value="Incomplete" <?php if ($student['status'] == 'Incomplete') echo 'selected'; ?>>Incomplete</option>
                            </select>
                        </td>
                    </tr>
                <?php endforeach; ?>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const attendanceSelects = document.querySelectorAll('.attendance');
            ...
            attendanceSelects.forEach(select => {
                select.addEventListener('change', function() {
                    ...
                    // Use AJAX to send the update to the server (update_attendance.php)
                    fetch('update_attendance.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/x-www-form-urlencoded',
                        },
                        body: `attendance_id=${attendanceId}&status=${newStatus}`,
                    })
                    .then(response => response.text())
                    .then(data => {
                        // Handle the response from the server (e.g., show a message)
                        console.log(data);
                        // You might want to update the displayed status in the table here
                        const statusSpan = this.previousElementSibling;
                        statusSpan.textContent = newStatus;
                        statusSpan.className = ''; //remove previous class

                        switch (newStatus) {
                            case 'Present':
                                statusSpan.classList.add('pres');
                                break;
                            case 'Late':
                                statusSpan.classList.add('late');
                                break;
                            case 'Absent':
                                statusSpan.classList.add('abse');
                                break;
                            case 'Incomplete':
                                statusSpan.classList.add('incom');
                                break;
                        }
                    })
                    .catch(error => {
                        console.error('Error:', error);
                        alert('Failed to update attendance status.'); // basic error message
                    });
                });
            });
